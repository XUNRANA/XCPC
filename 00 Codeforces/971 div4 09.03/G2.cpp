...
void solve()
{
	...
	for(int i=k;i<=n;i++)
	{
		...
		ans[i-k+1]=k-res;//代表区间ans[i-k+1,i]的答案 
	...
	{
		cin>>l>>r;
		q[l].push_back({r-k+1,i});
	}
	
	
	build(1,1,n);
//	for(int i=1;i<=4*n;i++) cout<<tr[i].l<<" "<<tr[i].r<<" "<<tr[i].s<<" "<<tr[i].lz<<"\n";
	stack<int>st;
	st.push(n-k+2);
...
//	cout<<"\n";
   ...
   //		cout<<"\n";
		for(auto j:q[i])
		{
   //			cout<<j.second<<" "<<i<<" "<<j.first-k+1<<" "<<query(1,i,j.first-k+1)<<"\n";
			ans1[j.second]=query(1,i,j.first).s;
		}
		st.push(i);
	}
