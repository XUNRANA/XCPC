...
void solve() 
{
    ...
    for(int i=0;i<(1<<k);i++) dp[i][0]=dp[i][1]=0;
    dp[0][0]=dp[0][1]=1;
    ...
    for(int i=1;i<=w;i++)
    {
        ...
        for(int s=0;s<(1<<k);s++) dp[s][j^1]=dp[s][j];
        ...
        for(int st=0;st<(1<<k);st++)
        {
            ...
            for(int s=0;s<(1<<k);s++)
            {
                if(f[h[i]][s]) dp[st|s][j^1]=1;
            }
        }
        j^=1;
    }

    for(int i=1;i<(1<<k);i++) if(dp[i][j]) ans=max(ans,(int)__builtin_popcount(i));
    cout<<k-ans<<"\n";
}
/*
